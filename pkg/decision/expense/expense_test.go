...
func TestCalc(t *testing.T) {  
	...
	for _, testCase := range testCasesSuccess {  
		t.Run(testCase.name, func(t *testing.T) {  
			...
			if val != testCase.expectedResult {  
				t.Fatalf("expected %s but got %s for expression %s", testCase.expectedResult, val, testCase.expression)  
			}  
		})  
	}  
	...
	for _, testCase := range testCasesFail {  
		t.Run(testCase.name, func(t *testing.T) {  
			val, err := Calc(testCase.expression)  
			if err == nil {  
				t.Fatalf("expected error for case %s but got result: %s", testCase.expression, val)  
			}  
